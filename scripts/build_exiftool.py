...
def build_exiftool(output_directory: Path) -> Path | None:
    ...
    configured_packer = os.environ.get("EXIFTOOL_PP")
    packer = configured_packer or shutil.which("pp")
    if packer is None:
        raise RuntimeError("PAR::Packer is required to build the bundled ExifTool")
    if not Path(packer).is_file():
        raise RuntimeError(f"PAR::Packer executable is unavailable: {packer}")
